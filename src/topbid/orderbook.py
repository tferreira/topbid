...
class OrderBook:
  ...
  def update(self, exchange_name: str, pairs: list):
    ...
    for _id, res in zip(ids, responses):
      ...
      if "result" in res:  # kraken
        ...
        self._set_price_and_volume(_id, res["result"][key]["bids"][0][0], res["result"][key]["bids"][0][1])
        continue
  ...
  def get_orderbook_bid_by_trade(self, trade: dict) -> tuple:
    ...
    if not price:
      ...
      logger.warning(f"Could not find bid price in orderbook_bids for {exchange_name=} {pair=}")
  ...
  def initialize_symbols_mappings(self, exchange_name):
    ...
    r = requests.get(url, headers={"authorization": f"Apikey {self.cmp_api_key}"})

    # If we got an error when querying the API, log and retry next time
    if r.status_code != 200:
      logger.warning(f"Failed to make a request to CryptoCompare API (HTTP{r.status_code})")
      return

    response = r.json()
    # If the response is not successful, log and retry next time
    if response["Response"] != "Success":
      logger.warning(f"Error from CryptoCompare API: {response['Message']}")
      return
    ...
    logger.info(f"Saved mappings from CryptoCompare API for exchange {exchange_name}")
